...
from erd import ERDGenerator, FieldMetadata, ModelMetadata, RelationshipMetadata
...
class TestERDPerformance:
    ...
    def test_performance_small_schema(self):
        ...
        # Mock a small schema
        with (
            patch.object(generator, "_discover_models"),
            patch.object(generator, "_extract_model_metadata"),
        ):
            # Create small schema
            user_metadata = ModelMetadata(
                ...
                line_number=1,
                fields=[
                    FieldMetadata(name=f"field_{i}", type_hint="str") for i in range(5)
                ],
                relationships=[],
                constraints=[],
            )
            ...
            assert (
                generation_time < 1.0
            ), f"Small schema took {generation_time:.2f} seconds"
        ...
        with (
            ...
            for i in range(10):
                ...
                model_metadata = ModelMetadata(
                    ...
                    fields=[
                        FieldMetadata(
                            name="id", type_hint="uuid.UUID", is_primary_key=True
                        )
                    ]
                    + [
            ...
            assert (
                generation_time < 5.0
            ), f"Medium schema took {generation_time:.2f} seconds"
            assert result is not None
            assert "erDiagram" in result
    ...
    def test_performance_large_schema(self):
        ...
        with (
            ...
            ), f"Large schema took {generation_time:.2f} seconds (requirement: <30s)"
            assert result is not None
            assert "erDiagram" in result
    ...
    def test_performance_complex_relationships(self):
        ...
        with (
            ...
            for i in range(15):
                ...
                model_metadata = ModelMetadata(
                    ...
                    fields=[
                        ...
                        FieldMetadata(
                            name="user_id", type_hint="uuid.UUID", is_foreign_key=True
                        ),
                    ],
                    relationships=[
            ...
            ), f"Complex relationships took {generation_time:.2f} seconds"
            assert result is not None
            assert "erDiagram" in result
    ...
    def test_performance_scaling_linear(self):
        ...
        for i, time_taken in enumerate(generation_times):
            assert (
                time_taken < 30.0
            ), f"Schema size {schema_sizes[i]} took {time_taken:.2f} seconds"
        ...
        for i in range(1, len(generation_times)):
            ...
            assert (
                ratio < 3.0
            ), f"Scaling ratio {ratio:.2f} is too high between sizes {schema_sizes[i-1]} and {schema_sizes[i]}"
    ...
    def test_performance_file_operations(self):
        ...
        try:
            ...
            assert (
                generation_time < 5.0
            ), f"File operations took {generation_time:.2f} seconds"
